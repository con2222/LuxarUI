...
struct vec<4> {
    ...
    constexpr vec(double x, double y, double z, double w)
        : x(x), y(y), z(z), w(w) {}
...
template <int n>
vec<n> operator*(const vec<n>& v1, const double scalar) {
    vec<n> result;
    for (int i = 0; i < n; i++) {
...
template <int n>
vec<n> operator*(const double scalar, const vec<n>& v1) {
    return v1 * scalar;
}
...
struct matrix {
    ...
    vec<C>& operator[](const int i) {
        assert(i >= 0 && i < R);
        return rows[i];
    }
    const vec<C>& operator[](const int i) const {
        assert(i >= 0 && i < R);
        return rows[i];
    }
...
inline mat4 mat4_make_perspective(float fov, float aspect, float znear,
                                  float zfar) {
    mat4 m = mat4::identity();
